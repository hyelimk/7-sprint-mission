...
-- 7. `orders` 테이블에서 커서 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일때,
--`id` 값을 기준으로 커서를 사용합시다. 커서의 값이 `42`일 때 다음 페이지를 조회하세요.
...
SELECT * FROM orders WHERE  time BETWEEN '00:00:00' AND '12:00:00'AND date = '2025-03-12';
...
SELECT pizza_id,order_id FROM order_details ;
-- 2. `order_details` 테이블에서 각 피자(`pizza_id`)별로 총 주문 수량을 구하세요.
SELECT pizza_id , quantity FROM order_details;
...
SELECT pizza_id, SUM(quantity) AS total FROM order_details GROUP BY pizza_id HAVING SUM(quantity) >=10 ORDER BY total ASC
-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. (수익 = `quantity * price`)
SELECT t.name, SUM(d.quantity * p.price) AS total_revenue FROM order_details d
JOIN pizzas p ON d.pizza_id = p.id JOIN pizza_types t ON p.type_id = t.id GROUP BY t.name ORDER BY total_revenue DESC;
-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date,COUNT(DISTINCT o.id)AS order_count, SUM(d.quantity) AS total_quantity FROM orders o
JOIN order_details d ON o.id = d.order_id GROUP BY o.date ORDER BY o.date;
-- # 고급 문제
...
SELECT
  p.*,
  s.total_quantity FROM pizzas p
JOIN (
  SELECT
    od.pizza_id,
...
) s ON s.pizza_id = p.id
ORDER BY s.total_quantity DESC LIMIT 10;
